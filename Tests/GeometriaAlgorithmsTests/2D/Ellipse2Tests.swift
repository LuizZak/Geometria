...
class Ellipse2Tests: XCTestCase {
    func testProjectApproximate_centerPoint() {
        let sut = makeSut(center: .zero, radius: .init(x: 10, y: 10))
        let point = Vector2D(x: 0, y: 0)

        let result = sut.projectApproximate(to: point, tolerance: 0.0001, samples: 20, maxDepth: 4)

        assertEqual(result, .init(x: 9.510565162951535, y: 3.090169943749474), accuracy: 0.001)
    }

    func testProjectApproximate_circleEquivalent() {
        let sut = makeSut(center: .zero, radius: .init(x: 10, y: 10))
    ...
    func testProjectApproximate_nonCircleEquivalent() {
        let sut = makeSut(center: .init(x: 2, y: 3), radius: .init(x: 12, y: 7))
        let point = Vector2D(x: 5, y: 7)

        let result = sut.projectApproximate(to: point, tolerance: 0.0001, samples: 20, maxDepth: 5)
